        ...
        public List<ImportDataModel> GetImportDataByExcel(string path)
        {
            ...
            foreach (DataRow row in dt.Rows)//遍历datatable,封装对象集合，遍历行然后取一列的值
            {
                list.Add(new ImportDataModel()
                {
                    ...
                    EFrontHoleLeftX1 = row["E前左X1"] != DBNull.Value ? Convert.ToDouble(row["E前左X1"]) : 0.0,
                    EFrontHoleLeftX2 = row["E前左X2"] != DBNull.Value ? Convert.ToDouble(row["E前左X2"]) : 0.0,
                    EFrontHoleRightX1 = row["E前右X1"] != DBNull.Value ? Convert.ToDouble(row["E前右X1"]) : 0.0,
                    EFrontHoleRightX2 = row["E前右X2"] != DBNull.Value ? Convert.ToDouble(row["E前右X2"]) : 0.0,
                });
            }
            return list;
        }
